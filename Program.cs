...
namespace C20_Ex01_1
{
    public class Program
    {
        ...
        {
            string[] binaryStringsBuilders = new string [i_SizeOfStringArray]; //allocates a string array which will hold the numbers from user's input.
            Console.WriteLine("please enter 4 numbers in a binary format, 8 digit each: ");
            for (int i = 0; i < i_SizeOfStringArray; i++)
            { 
                binaryStringsBuilders[i]= Console.ReadLine() ;
      ...
      public static bool checkIfInputIsValidRanged(string i_BinaryNums,char i_From,char i_To ,int i_LengthOfValidInput)// Checks if a string has 8 characters and only 1's and 0's.
        ...
        { 
            if (i_NumWhichIsPossiblePowerOfTwo!=0 &&(i_NumWhichIsPossiblePowerOfTwo & (i_NumWhichIsPossiblePowerOfTwo - 1)) == 0)// if the number is a power of two , then (number-1) will have lower-order bits set.
            {
                return true;
            }
            else
